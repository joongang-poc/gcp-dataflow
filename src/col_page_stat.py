...
def preProcessing(fields):
    ...
    if "sec_view" not in json_data:
        json_data["sec_view"] = None

    if "sec_ontime" not in json_data:
        json_data["sec_ontime"] = None
...
class PTransform(beam.DoFn) :
    ...
    def process(self, fields):
        ...
        ontime_variable = fields["ontime"]

        if type(fields["ontime"]) == type(com2):
            ontime_variable = fields["ontime"]["numberLong"]
        ...
        tablerow = {
                ...
                "uv": fields["uv"],
                "pv": fields["pv"],
                "ontime": ontime_variable,
                "read": fields["read"],
                "section_name": fields["section_name"],
                "bs": fields["bs"],
                "sec_view" : fields["sec_view"],
                "sec_ontime" : fields["sec_ontime"]
        }
        yield tablerow
